...
msgs = env['whatsapp.message'].sudo().search([
    ...
    ('state', 'in', ['cancel', 'error']),
])
print(f"Total failed/cancelled messages today: {len(msgs)}")
for m in msgs:
    ...
    print(f"  id={m.id} to={m.mobile_number} state={m.state} att={att} body={body_snippet} err={err}")
"""
