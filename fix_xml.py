...
# Replace Catalogue structure using regex
catalogue_pattern = re.compile(r'<div class="whatsapp-header sidebar-header-desktop" style="background: var\(--ws-bg-sidebar\);">\s*<span class="sidebar-title">Product Catalogue</span>\s*</div>', re.DOTALL)
new_catalogue = """<div class="whatsapp-header sidebar-header-desktop" style="background: var(--ws-bg-sidebar); display: flex; flex-direction: row; justify-content: space-between; align-items: center; padding: 20px;">
...
xml = catalogue_pattern.sub(new_catalogue, xml)

# Replace Settings structure completely
settings_pattern = re.compile(r'<t t-elif="state\.activeTab === \'settings\'">.*?</t>', re.DOTALL)
...
xml = settings_pattern.sub(new_settings, xml)

with open(xml_path, "w", encoding="utf-8") as f:
    f.write(xml)

print("XML replaced successfully.")
